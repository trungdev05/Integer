...
def main(argv: list[str]) -> int:
    ...
    calibration_measurements = bench.parse_calibration_payload(payload)

    baseline = bench.load_baseline(args.baseline) if args.baseline.exists() else {}
    baseline_times = baseline.setdefault("baseline_times", {})
    calibration_times = baseline.setdefault("calibration_times", {})
    scoring_system = baseline.setdefault("scoring_system", {
        "baseline_score": 200,
        "max_score": 1000,
        "description": "Score = baseline_score * (baseline_time / normalized_time), capped at max_score",
    })
    scoring_system["description"] = "Score = baseline_score * (baseline_time / normalized_time), capped at max_score"

    for item in measurements:
        baseline_times[str(item.digits)] = round(item.time_seconds, 9)
    for item in calibration_measurements:
        calibration_times[str(item.digits)] = round(item.time_seconds, 9)

    baseline["baseline_info"] = {
        "timestamp": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
        "seed": args.seed,
        "description": "Baseline and machine-calibration performance generated by scripts/update_baseline.py",
